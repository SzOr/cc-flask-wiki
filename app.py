...
def askwiki():
    ...
    if form.validate():
        if request.method == 'POST':
            ...
            if (counter > 0):
                repeated = SELECT ask_result FROM ask WHERE ask_query=name;
            ...
            if resp.ok:
                ...
                """
                INSERT INTO ask (ask_query, ask_result)
                VALUES (name, json_output)
                """, consistency_level=ConsistencyLevel.ONE)
                '''
                #POTENTIALLY NEED TO ADD EXECUTE TO THE STATEMENT ABOVE
    ...
    return render_template('form.html', form=form)#wiki=Wiki.query.all()
